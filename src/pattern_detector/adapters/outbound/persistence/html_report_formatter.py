"""Standalone, interactive Semantic UI (Fomantic-UI) HTML dashboard formatter for Erlang / OTP Pattern Detector."""
...
CATEGORY_STYLES = {
    ...
    PatternCategory.ACTOR_CONCURRENCY: {
        "color": "teal",
        "icon": "users",
...
<head>
    ...
    <title>🔴 DPX-Erlang: Architecture & OTP Dashboard - {project_name}</title>
...
<body>
    <div class="ui container">
            ...
            <div class="ui grid stackable middle aligned">
                <div class="ten wide column">
                    <h1 class="ui header inverted" style="margin: 0; font-size: 28px;">
                        🔴 DPX-Erlang: OTP Architecture & Pattern Dashboard
                        <div class="sub header" style="color: #fca5a5; margin-top: 6px;">
                            Static Analysis, OTP Behaviours, Actor Concurrency & Resilience for <strong>{project_name}</strong>
                        </div>
                    </h1>
                </div>
                <div class="six wide column right aligned">
                    <button id="copyLlmBtn" class="ui red button" onclick="copyArchMapForLlm()">
        ...
        <div class="ui grid stackable four column" style="margin-bottom: 20px;">
            <div class="column">
                <div class="kpi-card">
                    <div style="font-size: 28px; font-weight: 700; color: #f87171;">{total_detections}</div>
                    <div style="color: #a8a29e; font-size: 13px; margin-top: 4px;">Total Findings</div>
                </div>
            </div>
            <div class="column">
                <div class="kpi-card">
                    <div style="font-size: 28px; font-weight: 700; color: #fb923c;">{total_violations}</div>
                    <div style="color: #a8a29e; font-size: 13px; margin-top: 4px;">Violations / Smells</div>
                </div>
            </div>
            <div class="column">
                <div class="kpi-card">
                    <div style="font-size: 28px; font-weight: 700; color: #4ade80;">{total_patterns}</div>
                    <div style="color: #a8a29e; font-size: 13px; margin-top: 4px;">OTP & Actor Patterns</div>
                </div>
            </div>
            <div class="column">
                <div class="kpi-card">
                    <div style="font-size: 28px; font-weight: 700; color: #a78bfa;">{scanned_files} files</div>
                    <div style="color: #a8a29e; font-size: 13px; margin-top: 4px;">Scan Time: {elapsed_seconds}s</div>
                </div>
            </div>
        </div>

        <!-- Category Filter Pills -->
        <div class="ui secondary pointing menu inverted" style="border-color: #292524; margin-bottom: 16px; overflow-x: auto;">
        ...
        <div class="ui inverted segment" style="background: #1c1917; border: 1px solid #292524; border-radius: 8px; margin-bottom: 16px; padding: 10px 16px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
            <div class="ui mini inverted basic buttons" id="statusFilterGroup">
                <button class="ui button active status-filter-btn" data-status="all">
                    <i class="eye icon"></i> All <span class="ui mini red label" id="statusCountAll">{total_detections}</span>
                </button>
                <button class="ui button status-filter-btn" data-status="violation" style="color: #fb923c !important;">
                    <i class="exclamation triangle icon"></i> ⚠️ Violations <span class="ui mini orange label" id="statusCountViolation">{total_violations}</span>
                </button>
                ...
                <button class="ui button status-filter-btn" data-status="pattern" style="color: #f87171 !important;">
                    <i class="server icon"></i> 🔷 OTP & Behaviours <span class="ui mini red label" id="statusCountPattern">{total_patterns}</span>
                </button>
            </div>
            <div>
                <button id="principlesToggleBtn" class="ui mini inverted basic button" onclick="togglePrinciplesVisibility()" style="border-color: #dc2626; color: #fca5a5; font-weight: 600;">
                    <i class="shield alternate icon" style="color: #ef4444;"></i> <span id="principlesToggleText">Hide Principles & Smells</span>
                </button>
            </div>
        </div>
        ...
        <div class="ui fluid icon inverted input" style="margin-bottom: 20px;">
            <input type="text" id="searchInput" placeholder="🔎 Instant search by module, behaviour (gen_server, supervisor, gen_statem, pubsub, pool)..." style="background: #1c1917; border: 1px solid #292524; color: #fafaf9; padding: 12px 16px;">
        ...
        <div id="noViolationsAlert" class="ui positive icon message" style="display: none; background: rgba(34, 197, 94, 0.12); border: 1px solid rgba(34, 197, 94, 0.35); color: #86efac; margin-bottom: 20px; border-radius: 8px;">
            ...
            <div class="content">
                <div class="header" style="color: #4ade80; font-size: 16px; font-weight: 700;">Zero Violations Found!</div>
                <p style="color: #d6d3d1; margin-top: 4px;">All evaluated Erlang modules adhere cleanly to OTP standards and Let-It-Crash resilience guidelines.</p>
            </div>
        </div>

        <!-- No Matching Results Message -->
        <div id="noResultsMessage" class="ui inverted segment" style="display: none; background: #1c1917; border: 1px solid #292524; text-align: center; padding: 30px; border-radius: 8px;">
            <i class="search icon" style="font-size: 24px; color: #78716c; margin-bottom: 10px;"></i>
            <div style="color: #a8a29e; font-size: 15px;">No findings match the selected category, action status, or search query.</div>
        </div>
...
class HtmlReportFormatter(ReportFormatterPort):
    """Renders a standalone, responsive, interactive Semantic UI HTML dashboard for Erlang / OTP DetectionReport."""
    ...
    def format(self, report: DetectionReport, include_principles: bool | None = None) -> str:
        ...
        project_name = html.escape(os.path.basename(os.path.abspath(report.project_path)) or "Erlang Project")
    ...
    def _classify_detection_status(self, det: Detection) -> str:
        if det.pattern_category == PatternCategory.SAFETY:
            return "violation"
        if det.pattern_category == PatternCategory.RESILIENCE and det.pattern_type == PatternType.SUPERVISOR_RESTART_STORM:
            return "violation"
        if det.pattern_category == PatternCategory.PRINCIPLE:
            if det.pattern_type in (PatternType.BEHAVIOUR_CALLBACK_COMPLIANCE,):
                return "violation"
            return "violation"
        if det.pattern_type == PatternType.CIRCULAR_MODULE_DEPENDENCY:
            return "violation"
        return "pattern"
    ...
    def _render_cards_list(self, detections: list[Detection]) -> list[str]:
        ...
        for idx, det in enumerate(detections, 1):
            ...
            cat_style = CATEGORY_STYLES.get(det.pattern_category, CATEGORY_STYLES[PatternCategory.OTP_BEHAVIOUR])
            ...
            for ev in det.evidences:
                ...
                evidences_html.append(
                    ...
                    <div style="margin-top: 4px;">
                        <span style="color: #ef4444; font-weight: 600;">+{int(ev.weight * 100)}%</span>
                ...
                <div class="pattern-card" data-category="{det.pattern_category.value}" data-pattern="{det.pattern_type.value}" data-status="{status}" data-target="{html.escape(det.target_name.lower())}">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            ...
                            <strong style="font-size: 16px; margin-left: 8px; color: #fafaf9;">{html.escape(det.target_name)}</strong>
                            <span style="color: #a8a29e; font-size: 13px;">({det.target_kind})</span>
                        </div>
                        <div>
                    ...
                    <div style="margin-top: 10px; font-size: 13px; color: #e7e5e4;">
    ...
    def _build_llm_architectural_map(
        ...
        lines = [
            "# 🔴 DPX-Erlang: Codebase Architecture Map & Refactoring Analysis",
            "",
            "## 📌 Project Overview",
            ...
            f"- **🔷 OTP Behaviours & Patterns:** `{counts.get('pattern', 0)}`",
            f"- **✅ Clean Adherences:** `{counts.get('adherence', 0)}`",
            "",
            "---",
            "",
            "## 🎯 Task for AI / LLM Erlang / OTP Architect",
            "> **Prompt Instructions:**",
            "> 1. **Analyze OTP Tree Architecture:** Review supervisors, gen_server state management, and worker pools.",
            "> 2. **Review Resilience & Failures:** Audit Let-It-Crash compliance, selective receive mailbox leaks, and blocked handle_calls.",
            "> 3. **Propose Idiomatic OTP Refactorings:** Suggest gen_statem, pg pub/sub, or ETS caching architectures with concrete Erlang module skeletons.",
            "> 4. **Safety Improvements:** Eliminate dynamic atom creation (`list_to_atom`) and defensive catch-alls (`catch _:_`).",
            "",
            "---",
            "",
        ]
        ...
        # 1. OTP Behaviours & Patterns
        lines.append(f"## 🔷 Active OTP Behaviours & Design Patterns ({counts.get('pattern', 0)} instances)")
        if patterns_by_type:
