"""Typer CLI interface for DPX-Erlang."""
...
app = typer.Typer(
    name="dpx-erlang",
    help="🔴 Hexagonal Pattern Scanner & Architecture Detector for Erlang / OTP (OTP 20 - 27+).",
...
def scan(
    path: Annotated[
        str,
        typer.Argument(
            help="Path to an Erlang source file (.erl / .hrl) or directory containing an Erlang app / release.",
        ),
    ] = ".",
    ...
    pattern: Annotated[
        ...
        typer.Option(
            "--pattern",
            "-p",
            help="Filter for specific pattern type(s) (e.g. 'gen_server', 'supervisor', 'actor_model', 'worker_pool_otp').",
        ),
    ] = None,
    ...
    no_principles: Annotated[
        ...
        typer.Option(
            "--no-principles",
            help="Exclude quality principles and clean code smells (OTP Behaviours & Concurrency only).",
        ),
    ] = False,
    exclude: Annotated[
        list[str] | None,
    ...
    """Scan an Erlang project or source file for OTP behaviours, actor concurrency, and resilience smells."""
...
def _handle_terminal_scan(
    ...
    # 1. Header Banner
    header = Panel.fit(
        f"[bold white]🔴 DPX-Erlang: OTP Pattern Scanner & Architecture Detector • Hexagonal DDD[/bold white]\n"
    ...
    table = Table(title="📊 Detection Summary by Category", header_style="bold red")
    ...
    console.print("[bold]📋 Identified Design Patterns & Behaviours:[/bold]\n")
    for idx, d in enumerate(report.detections, 1):
        ...
        header_text = f"[bold red]#{idx} {d.pattern_type.value.upper()}[/bold red] on [bold]{d.target_kind}[/bold] '[yellow]{d.target_name}[/yellow]'"
        ...
        for i, ev in enumerate(d.evidences):
            ...
            lines.append(f"{connector} [red]+{int(ev.weight * 100)}%[/red] ([dim]{ev.rule_code}[/dim]) {ev.description}{ev_loc}")
...
@app.command()
def rules() -> None:
    """List all supported Erlang / OTP design patterns, behaviours, and resilience rules."""
    table = Table(title="🔴 Supported Patterns & Rules in DPX-Erlang", header_style="bold red")
...
@app.command()
def info() -> None:
    """Display DPX-Erlang architecture, version, and grammar engine details."""
    panel = Panel(
        """[bold red]🔴 DPX-Erlang • OTP Pattern Scanner & Architecture Detector[/bold red]
[dim]Multi-Paradigm Hexagonal Pattern Detector & Erlang AST Engine[/dim]

• [bold]Target Language:[/bold] Erlang / OTP (OTP 20 - 27+ edition)
• [bold]Architecture:[/bold] Hexagonal Ports & Adapters + Domain-Driven Design (DDD)
• [bold]Rules Supported:[/bold] 30 Rules (gen_server, supervisor, gen_statem, gen_event, pg pubsub, poolboy, let-it-crash)
